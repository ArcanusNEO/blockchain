...
def alice_swap_tx(txid_to_spend, utxo_index, amount_to_send):
  txout_script = coinExchangeScript(
      alice_public_key_BTC, bob_public_key_BTC, hash_of_secret())
...
def return_coins_tx(amount_to_send, last_tx, lock_time, script):
  txin = create_txin(b2lx(last_tx.GetHash()), 0)
...
def complete_return_tx(return_coins_tx, txin_scriptPubKey, bob_signature_BTC):
  ...
  txin.scriptSig = CScript(coinExchangeScriptSig2(
      alice_signature_BTC, bob_signature_BTC))
